...
export default function Rightbar({ user }) {
  ...
  useEffect(() => {
    const getFriends = async () => {
      try {
        const friendList = await axios.get("https://unituit-api.herokuapp.com/users/friends/" + user._id);
  ...
  const handleClick = async () => {
    try {
      if (followed) {
        await axios.put(`https://unituit-api.herokuapp.com/users/${user._id}/unfollow`, {
          userId: currentUser._id,
        });
        dispatch({ type: "UNFOLLOW", payload: user._id });
      } else {
        await axios.put(`https://unituit-api.herokuapp.com/users/${user._id}/follow`, {
          userId: currentUser._id,
        });
